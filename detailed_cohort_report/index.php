...
<head>
    ...
    <?php echo '<link rel="stylesheet" type="text/css" href="' . $CFG->wwwroot . '/local/detailed_cohort_report/styles.css">'; ?>
    <style>
 ...
 *
 * @package   local_detailed_cohort_report
...
use core_group\reportbuilder\local\entities\group;
 ...
 $PAGE->set_title('Detailed Grader Report by Cohort');
 $PAGE->set_heading('Detailed Grader Report by Cohort');
 $PAGE->set_url(new moodle_url('/local/detailed_cohort_report/index.php'));
...
// Function to replace duration in days with duration in days and weeks
/*
function calculateDurationInWeeksAndDays($timestart, $timeend) { // Alternative function that can replace calculatedurationindays
...
function calculateDurationInDays($timestart, $timeend) {
    // Calculate the duration in seconds
    if ($timestart == 0 and $timeend == 0){
        $time = "Not Applicable";
        return $time;
...
// Function used to get enroll date, completion date and duration for a student in a course
function get_dates($userid, $courseid){ // Function to get start date and end date for a course, start date is retrieved from enrollment info and end date from course completion info
    global $DB;
    $sql = "SELECT ue.userid, ue.timestart AS uenroltimestart, ue.timecreated AS uenroltimecreated, e.timecreated AS enroltimecreated, ue.timeend AS uetimeend
    FROM {enrol} AS e
    ...
    $sql = "SELECT cc.userid ,u.id, cc.timecompleted AS timecomplete
    ...
    WHERE u.id = :userid
    AND c.id = :courseid";
    $completedate = $DB->get_records_sql($sql, $params);
...
// Display the dropdown menu for selecting group name
...
$selectedGroupName = $selected_group_name; // Replace with the actual selected group name

// Echo the selected group name as a JavaScript variable to be used in excel sheet name
...
$selected_group_ids = array(); // ALL group ids for selected group in each course (Each course has different groupid for same name group)
...
$selectedusers = array();  // Now we have info of each user who is in the group selected.(Duplicates removed)
...
$courseslist = array(); // First list of courses (to be filtered further to not include course managers courses)
$courseslist1 = array(); // Dummy array
foreach ($selectedusers as $use){
    $courses_list_student = get_courses_for_student($use -> id); // Courses of student selected
    foreach($courses_list_student as $course=>$id){
        if(!in_array($id , $courseslist1)){ // If student takes a course that is not already in the course add it's id to the list
            $courseslist1 [] = $id;
        }
    }
}
foreach ($courseslist1 as $course=>$value){ // Load complete course objects into $courseslist array
    $val = $value->id;
...
echo '<tbody>';
foreach ($selectedusers as $selecteduser) { // Each user's row includes grade, startdate, enddate, duration
    echo '<tr>';
    echo '<td id="sticky-column"><a href="/user/profile.php?id=' . $selecteduser->id . '">' . $selecteduser->firstname . " " . $selecteduser->lastname . '</a></td>';
    //echo '<td>' . $selecteduser->email . '</td>'; // For email addresses column
    ...
    foreach ($courseslist as $course) {
        if ($course->groupid != ''){
            ...
            echo $grade;
            echo '</td>';
            $dates = get_dates($selecteduser->id, $course->id);
            echo '<td>';
            echo $dates['startdate'];
            echo '</td>';
            echo '<td>';
            echo $dates['enddate'];
            echo '</td>';
...
$(document).ready(function() {
        ...
        buttons: [
            {
                ...
                filename: function() {
                    ...
                    return 'Grade report - Group: ' + selectedGroupName + ' ' + timestamp; // Name of exported excel sheet
                }
            }
        ]
    });
    ...
    //$("[name='gradeTable_length']").css('width', 64); // Alreading in css file
